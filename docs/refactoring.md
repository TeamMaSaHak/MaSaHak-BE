...
- **해결책**: 우선은 number로 진행 후에 추후에 규모가 커질 시 String 변경
...
### 1. 배치 쿼리 성능 최적화 (미래 과제)

- **현재**: `pushNotifications()`와 `hourlyBatchRunner()`에서 루프 내 다중 DB 쿼리 실행
- **문제**: 유저 수 증가 시 쿼리 부하 발생 가능
- **제안**:
  - 타임존별로 유저를 그룹화하여 bulk query 처리
  - 또는 배치 처리로 한 번에 다수 유저 처리
- **파일**: `src/modules/batch/batch.service.ts`
- **우선순위**: 낮음 (현재 유저 규모에서는 문제없음, 추후 최적화)
...
- PR #19: https://github.com/TeamMaSaHak/MaSaHak-BE/pull/19
